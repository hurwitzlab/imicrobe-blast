...
while read FILE; do
  fasplit.py -f $FILE -o $SPLIT_DIR -n $MAX_SEQS_PER_FILE
done < $INPUT_FILES
...
while read INPUT_FILE; do
  ...
  if [[ ${#BLAST_TO_DNA} -gt 0 ]]; then
    while read DB; do
      BASE_DB=$(basename $DB)
      echo "$BLAST_TO_DNA $BLAST_ARGS -perc_identity $PCT_ID -db \"$BLAST_DIR/$BASE_DB\" -query \"$INPUT_FILE\" -out \"${BLAST_OUT_DIR}/${BASENAME}-${BASE_DB}.tab\"" >> $BLAST_PARAM
    done < $BLAST_DBS
  fi
...
echo "annotate.py -b \"$BLAST_OUT_DIR\" -a \"${WORK}/imicrobe/annotations/annots.db\" -o \"${OUT_DIR}/annotations\"" > $ANNOT_PARAM
